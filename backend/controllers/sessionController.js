...
const createSession = asyncHandler(async (req, res) => {
    ...
    if (!practiceMode || !level || !interviewType || !count) {
        res.status(400);
        throw new Error(
            'Please specify practice mode, level, interview type, and question count.'
        );
    }

    // Role is required only for role-based interview
    if (practiceMode === 'role-based' && !role) {
        res.status(400);
    ...
    let session = await Session.create({
        ...
        // Keep a useful display value for existing UI/evaluation logic
        role:
            practiceMode === 'role-based'
                ? role
                : practiceMode === 'sde-mixed'
                    ? 'SDE Mixed Interview'
                    : topic || role,
        ...
        // Add these only if they exist in your Session schema
        // practiceMode,
        // topic,
    });
        ...
        try {
            ...
            const codingCount =
                interviewType === 'coding-mix'
                    ? Math.max(1, Math.floor(Number(count) * 0.2))
                    : 0;

            const questionsArray = aiData.questions.map(
                (qText, index) => ({
                    questionText: qText,
                    questionType:
                        index < codingCount ? 'coding' : 'oral',
                    isEvaluated: false,
                    isSubmitted: false,
                })
            );
